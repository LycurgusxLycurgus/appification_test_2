...
// Create a class to manage the 2D task arena
class TaskArena2DManager {
  constructor(canvas, ctx, state, requiredScore, onCompleteCallback) {
    console.log("TaskArena2DManager constructor");
    
    // Get quality settings
    this.quality = gameState.graphicsQuality || 'medium';
    this.qualitySettings = QUALITY_SETTINGS[this.quality];
    console.log(`Using ${this.quality} quality settings for 2D task arena`);
    ...
    // Store required score and callback
    this.requiredScore = requiredScore || 300;
    this.onCompleteCallback = onCompleteCallback;
    
    // Add flag to track if callback has been executed
    ...
    this.targets = [];
    this.score = 0;
    this.timeRemaining = TASK_CHALLENGE_TIME;
    this.isActive = false;
    this.lastUpdateTime = 0;
    this.activeEffects = [];
    this.exitPortal = null;
    this.scoreGoalReached = false;
    
    // Track last shoot time for cooldown
    ...
    window.taskArena2DManager = this;
    
    console.log("TaskArena2DManager created with required score:", this.requiredScore);
  }
  
  // Setup a direct L key listener that bypasses gameState
  setupLKeyListener() {
    console.log("Setting up L key listener for task arena");
    
    this.lKeyHandler = (e) => {
      if (!this.isActive) return;
      
      if (e.key.toLowerCase() === 'l') {
        console.log("L key pressed in task arena");
    ...
    console.log("L key listener added for task arena");
  }
  
  // Initialize the task arena
  initialize() {
    console.log("Initializing 2D task arena");
    this.isActive = true;
    this.score = 0;
    this.timeRemaining = TASK_CHALLENGE_TIME;
    ...
    try {
      ...
      console.log("2D task arena initialized with time:", this.timeRemaining);
    } catch (error) {
      console.error("Error initializing 2D task arena:", error);
      this.exitArena();
    }
  }
  
  // Create platforms for the task arena
  createTaskPlatforms() {
  ...
  createBullet() {
    ...
    console.log("Creating bullet in 2D task arena");
  ...
  update(deltaTime) {
    ...
    try {
      // If we're in the ending state, we still want to render but don't update game elements
      if (this.isEnding) {
      ...
      // Check if score goal has been reached
      if (this.score >= this.requiredScore && !this.scoreGoalReached) {
        this.scoreGoalReached = true;
        this.createScoreGoalReachedEffect();
      }
      
      // Handle shooting with L key directly in the update loop
      // This is a backup method in case the key listener doesn't work
      if (this.gameState.keys && this.gameState.keys['l']) {
        const currentTime = Date.now();
        if (currentTime - this.lastShootTime > this.shootCooldown) {
          this.lastShootTime = currentTime;
          this.createBullet();
          // Reset the key state to prevent continuous shooting
          this.gameState.keys['l'] = false;
        }
      }
      
      return true;
    } catch (error) {
      console.error("Error updating 2D task arena:", error);
      return false;
    }
  }
  
  // Create score goal reached effect
  createScoreGoalReachedEffect() {
    // Create a special effect when the score goal is reached
    const effect = {
  ...
  updatePlayerMovement(deltaTime) {
    ...
    if (this.gameState.playerPosition.x < 0) {
      this.gameState.playerPosition.x = 0;
    } else if (this.gameState.playerPosition.x > this.canvas.width - 40) {
      this.gameState.playerPosition.x = this.canvas.width - 40;
    }
  }
  ...
  updateEffects() {
    ...
    for (let i = 0; i < this.activeEffects.length; i++) {
      ...
      } else if (effect.type === 'goal') {
        effect.opacity = 1 - lifePercent;
        effect.size = 50 * (1 + lifePercent);
      }
    }
  ...
  // Show final score
  showFinalScore() {
    console.log("Showing final score:", this.score, "Required:", this.requiredScore);
    ...
    const isSuccess = this.score >= this.requiredScore;
    
    // If we have a callback and it hasn't been executed yet, call it with the score
    if (this.onCompleteCallback && typeof this.onCompleteCallback === 'function' && !this.callbackExecuted) {
      console.log("Executing completion callback with score:", this.score);
      this.callbackExecuted = true;
      this.onCompleteCallback(this.score);
    }
  }
  
  // Render the task arena
  render() {
    if (!this.isActive) {
      console.warn("Render called but task arena is not active");
      return;
    }
    
    try {
      // Debug info
      console.log("TaskArena2DManager.render: Starting render with canvas:", 
                  this.canvas ? `${this.canvas.width}x${this.canvas.height}` : 'null',
                  "context:", this.ctx ? 'valid' : 'null');
      
      // Clear canvas and draw background
      this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
      
      // Draw background gradient - updated to match trainingArena2DSetup.js
      ...
      // Draw a debug marker to verify rendering is happening
      this.ctx.fillStyle = '#FF0000';
      this.ctx.fillRect(10, 10, 20, 20);
      this.ctx.fillStyle = '#FFFFFF';
      this.ctx.font = '12px Arial';
      this.ctx.fillText(`Frame: ${Date.now()}`, 40, 20);
      
      // If we're in the ending state, show the final score overlay
      if (this.isEnding) {
        ...
        // Show different messages based on success or failure
        if (this.score >= this.requiredScore) {
          this.ctx.fillStyle = '#4CAF50'; // Green for success
          this.ctx.fillText("Challenge Completed!", this.canvas.width / 2, this.canvas.height / 2 - 20);
        } else {
          this.ctx.fillStyle = '#F44336'; // Red for failure
          this.ctx.fillText("Challenge Failed", this.canvas.width / 2, this.canvas.height / 2 - 20);
        }
        
        this.ctx.fillStyle = '#FFFFFF';
        this.ctx.fillText(`Your Score: ${this.score}`, this.canvas.width / 2, this.canvas.height / 2 + 20);
        this.ctx.fillText(`Required Score: ${this.requiredScore}`, this.canvas.width / 2, this.canvas.height / 2 + 60);
        
        this.ctx.font = '20px Arial';
        this.ctx.fillText('Exiting in 5 seconds...', this.canvas.width / 2, this.canvas.height / 2 + 100);
        return;
      }
      ...
      // Draw exit portal with pulsing effect - updated to match trainingArena2DSetup.js
      if (this.exitPortal) {
      ...
      // Debug info - confirm render completed
      console.log("TaskArena2DManager.render: Completed render");
    } catch (error) {
      console.error("Error rendering 2D task arena:", error);
    }
  }
  
  // Draw UI elements (score, timer, goal)
  drawUI() {
    // Draw score
    ...
    // Draw score goal
    this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    this.ctx.fillRect(20, 70, 150, 40);
    
    // Change color based on progress
    if (this.score >= this.requiredScore) {
      this.ctx.fillStyle = '#4CAF50'; // Green when goal reached
    } else if (this.score >= this.requiredScore * 0.7) {
      this.ctx.fillStyle = '#FFC107'; // Yellow when close
    } else {
      this.ctx.fillStyle = '#FFFFFF'; // White otherwise
    }
    
    this.ctx.font = '20px Arial';
    this.ctx.textAlign = 'left';
    this.ctx.fillText(`Goal: ${this.requiredScore}`, 30, 95);
    ...
    const progressY = this.canvas.height - 40;
    ...
    // Draw progress fill
    if (progress >= 1) {
      this.ctx.fillStyle = '#4CAF50'; // Green when complete
    } else if (progress >= 0.7) {
      this.ctx.fillStyle = '#FFC107'; // Yellow when close
    } else {
      this.ctx.fillStyle = '#F44336'; // Red otherwise
    }
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText(`${Math.floor(progress * 100)}%`, progressX + progressWidth / 2, progressY + 15);
  }
  
  // Exit arena and return to task game
  exitArena() {
    console.log("Exiting 2D task arena");
    
    try {
      // Set inactive flag to prevent further updates
      this.isActive = false;
      
      // Remove L key event listener
      if (this.lKeyHandler) {
        window.removeEventListener('keydown', this.lKeyHandler);
        this.lKeyHandler = null;
      }
      ...
      // CRITICAL CHANGE: Only call the callback if it hasn't been executed yet
      if (this.onCompleteCallback && !this.callbackExecuted) {
        console.log("Calling completion callback with score:", this.score);
        this.callbackExecuted = true;
        this.onCompleteCallback(this.score);
        // Do NOT transition to 2D mode here - let the callback handle it
      } else if (this.callbackExecuted) {
        console.log("Completion callback already executed, skipping");
      } else {
        // Only as a fallback if no callback was provided, transition to 2D mode
        console.log("No completion callback provided, falling back to direct 2D transition");
        import('./modeTransitionManager').then(({ transitionGameMode }) => {
          transitionGameMode('2D', {}, () => {
            console.log("Successfully transitioned back to 2D mode from task arena");
          });
        }).catch(error => {
          ...
          if (this.gameState) {
            ...
            if (this.gameState.setMode) {
              this.gameState.setMode('2D');
            }
          }
        });
      }
      
      // Remove global reference to arena manager
      if (window.taskArena2DManager === this) {
        window.taskArena2DManager = null;
      }
    } catch (error) {
      console.error("Error during task arena exit:", error);
      
      // Ensure global reference is removed even if there's an error
      if (window.taskArena2DManager === this) {
        window.taskArena2DManager = null;
      }
      
      // Emergency fallback to 2D mode
      if (this.gameState) {
        this.gameState.mode = '2D';
      }
    }
  ...
  // Clean up resources
  cleanup() {
    console.log("Cleaning up task arena resources");
    
    try {
      // Remove custom event listener for L key
      if (this.lKeyHandler) {
        window.removeEventListener('keydown', this.lKeyHandler);
        this.lKeyHandler = null;
      }
      
      // Clear bullets, effects, and targets
      this.gameState.bullets = [];
      this.activeEffects = [];
      this.targets = [];
      ...
      console.error("Error during task arena cleanup:", error);
    }
  }
}

// Create task arena manager instance
let taskArena2DManager = null;

// Function to shoot in task arena - can be called from outside
export function shootInTaskArena() {
  if (taskArena2DManager && taskArena2DManager.isActive) {
    console.log("External call to shoot in task arena");
    taskArena2DManager.createBullet();
    return true;
  }
  return false;
}

// Function to initialize the 2D task arena
export function enterTaskArena2D(canvas, ctx, state, requiredScore, onCompleteCallback) {
  console.log("Entering 2D task arena with canvas:", canvas ? `${canvas.width}x${canvas.height}` : 'null', 
              "context:", ctx ? 'valid' : 'null',
              "mode:", state.mode);
  
  try {
    // Clean up any existing instance
    if (taskArena2DManager) {
      console.log("Cleaning up existing task arena manager");
      taskArena2DManager.cleanup();
    }
    
    // Verify canvas and context
    if (!canvas || !ctx) {
      console.error("Canvas or context is null/undefined in enterTaskArena2D");
      return null;
    }
    
    // Create new instance
    taskArena2DManager = new TaskArena2DManager(canvas, ctx, state, requiredScore, onCompleteCallback);
    
    // Initialize arena
    taskArena2DManager.initialize();
    
    // Log the current state of the arena
    console.log("Task arena initialized with:", {
      canvasWidth: canvas.width,
      canvasHeight: canvas.height,
      isActive: taskArena2DManager.isActive,
      targetCount: taskArena2DManager.targets.length,
      requiredScore: taskArena2DManager.requiredScore
    });
    
    // Set game mode to 2D_TASK_ARENA - we're already in this mode from the direct transition
    // but we'll set it again to be sure
    state.previousMode = state.mode !== '2D_TASK_ARENA' ? state.mode : (state.previousMode || 'TASK_GAME');
    state.mode = '2D_TASK_ARENA';
    
    console.log("Game mode confirmed as:", state.mode);
    
    return taskArena2DManager;
  } catch (error) {
    console.error("Error entering 2D task arena:", error);
    // If there's an error, make sure we return to TASK_GAME mode
    import('./modeTransitionManager').then(({ transitionGameMode }) => {
      transitionGameMode('TASK_GAME', {}, () => {
        console.log("Returned to TASK_GAME mode after error in task arena initialization");
      });
    }).catch(() => {
...
// Function to update the 2D task arena
export function updateTaskArena2D(deltaTime) {
  try {
    if (taskArena2DManager && taskArena2DManager.isActive) {
      const result = taskArena2DManager.update(deltaTime);
      // Add more detailed logging for debugging
      if (!result) {
        console.warn("updateTaskArena2D returned false - arena may be inactive or ending");
      }
      return result;
    } else {
      if (taskArena2DManager) {
        console.warn("Task arena manager exists but is not active");
      } else {
        console.warn("Task arena manager does not exist");
      }
    }
  } catch (error) {
    console.error("Error updating 2D task arena:", error);
...
// Function to render the 2D task arena
export function renderTaskArena2D() {
  try {
    if (taskArena2DManager && taskArena2DManager.isActive) {
      // Add more detailed logging for debugging
      console.log("Rendering task arena with canvas:", 
                  taskArena2DManager.canvas ? `${taskArena2DManager.canvas.width}x${taskArena2DManager.canvas.height}` : 'null',
                  "context:", taskArena2DManager.ctx ? 'valid' : 'null');
      
      taskArena2DManager.render();
      return true;
    } else {
      if (taskArena2DManager) {
        console.warn("Task arena manager exists but is not active during render");
      } else {
        console.warn("Task arena manager does not exist during render");
      }
    }
  } catch (error) {
    console.error("Error rendering 2D task arena:", error);
...
// Function to cleanup the 2D task arena
export function cleanupTaskArena2D() {
  console.log("Cleaning up 2D task arena from external call");
  try {
    if (taskArena2DManager) {
      taskArena2DManager.cleanup();
      taskArena2DManager = null;
    }
  } catch (error) {
    console.error("Error cleaning up 2D task arena:", error);
  }
}

// Add a direct function to handle L key presses from the input manager
export function handleLKeyInTaskArena() {
  if (taskArena2DManager && taskArena2DManager.isActive) {
    taskArena2DManager.createBullet();
    return true;
  }
